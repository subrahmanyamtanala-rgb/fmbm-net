...
The FLCAT gradient payload per client per round is:
\begin{equation}
P_{\mathrm{FMBM}} = |\boldsymbol{\theta}_{\mathrm{FLCAT}}| \times 4\;\text{bytes}
= 1{,}540 \times 4 = 6\;\text{KB},
\label{eq:payload}
\end{equation}
where $|\boldsymbol{\theta}_{\mathrm{FLCAT}}|\!=\!1{,}540$ parameters covers
the $Q/K/V/O$ projection matrices across $M\!=\!3$ modalities and $H\!=\!8$
heads. Standard FL methods (FedAvg, FedProx, MOON) transmit the full model
gradient at $28{,}075{,}520$ parameters $\times\!4\!=\!112.3$~MB/round.
The 66,845$\times$ reduction is therefore a direct consequence of federating
only the cross-modal attention layer, not full gradient aggregation.
This is architecturally consistent with Eq.~(\ref{eq:fedavg}), which
aggregates $\boldsymbol{\theta}$; here $\boldsymbol{\theta}$ refers
specifically to $\boldsymbol{\theta}_{\mathrm{FLCAT}}$, not the full
model weights.
