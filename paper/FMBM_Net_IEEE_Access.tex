...
\item \textbf{RSGC}: A deterministic four-tier confidence finite-state
  machine generating regulatory compliance audit trails designed to align
  with the BMJ risk-stratified framework~\cite{bmj_governance_2026} and
  EU AI Act~\cite{eu_ai_act_2024}.
...
Fig.~\ref{fig:timeline} illustrates the evolution of FL methods from
FedAvg~\cite{mcmahan2017} (2017) through FMBM-Net (2026). \cite{rieke2020fedhealth} provided the foundational survey of FL in digital
...
The key distinction is that FMBM-Net is, to our knowledge, the first
federated architecture to simultaneously support all three modalities
...
client --- two orders of magnitude below gradient-level FL.
...
tamper-evident audit trail. The RSGC's contribution must be assessed on
governance dimensions: \textbf{(i) auditability} --- every inference
produces a tamper-evident record; \textbf{(ii) regulatory alignment} ---
the four-tier FSM aligns with EU AI Act and HIPAA obligations; and
\textbf{(iii) risk stratification} --- the RSGC routes 11.3\% of CheXpert
predictions to mandatory physician review with precision 0.94 and recall 0.91.
...
Phase~2 optimizes the global FLCAT weights via size-weighted FedAvg~\cite{mcmahan2017}:
\begin{equation}
...
\label{eq:fedavg}
\end{equation}
over $T\!=\!100$ communication rounds. The complete procedure is summarised
...
\STATE \textbf{Phase 1:} For each client $c$: pre-train GEN, RUE-imaging,
       EHR branches via $\mathcal{L}_{\mathrm{local}}$, AdamW, batch 32,
       30 epochs.
\STATE \textbf{Phase 2:} For $t = 1,\ldots,T$:
\STATE \quad Server broadcasts $\boldsymbol{\theta}^{(t)}$ to all clients.
\STATE \quad Each client: compute $\mathbf{z}_g,\mathbf{z}_i,\mathbf{z}_e$;
       add DP noise (Eq.~\ref{eq:dp}); forward through FLCAT; compute gradient.
\STATE \quad Server: aggregate via Eq.~(\ref{eq:fedavg}).
\ENSURE $\boldsymbol{\theta}^{(T)}$ with $(\varepsilon\!=\!2.0,
        \delta\!=\!10^{-5})$-DP guarantee.
\end{algorithmic}
\end{algorithm}
...
Table~\ref{tab:commcost} translates round count into concrete communication
volume and wall-clock time. FMBM-Net transmits 6~KB latent codes per round
versus 112.3~MB full gradient vectors for standard FL, achieving a
66,845$\times$ reduction in total transmitted bytes to reach equivalent
accuracy.
...
during training \cite{ma2021smil} may improve robustness. (iii) All
