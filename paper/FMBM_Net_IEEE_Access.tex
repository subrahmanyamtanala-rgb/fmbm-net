...
but these \emph{do not traverse the network}: FLCAT is a server-side
computation operating on received latent codes. Standard FL methods
(FedAvg, FedProx, MOON) transmit the full-model gradient at each round,
totalling 112.3~MB. The substantially reduced uplink payload is a direct
consequence of this server-side FLCAT design, not gradient compression.
Note: the 66,845$\times$ total-payload reduction in Table~\ref{tab:commcost}
combines both the per-round uplink saving ($19{,}166\!\times$) and the
reduced round count (28 vs.~100 rounds) to reach equivalent accuracy.
