...
clinical trajectory including smoking history, medication exposures, and prior oncology encounters from the EHR. Individually, none of these streams provides sufficient evidence for a confident clinical decision. Together, their integration can shift the predicted malignancy probability from 60\,\% (imaging alone) to 89\,\% (multimodal), directly influencing the recommendation from surveillance to tissue biopsy or surgical resection. \textit{(Illustrative values only; not experimentally measured FMBM-Net outputs. See Section~\ref{sec:experiments} for evaluated benchmarks.)} \textit{(These probability values are illustrative of the clinical motivation and do not represent experimentally measured outcomes from FMBM-Net; the actual system is evaluated on the four public benchmarks described in Section~\ref{sec:experiments}.)}
...
in precision oncology with adaptive privacy budgets (note: this work was
under review at time of writing; all five 2024--2026 multimodal FL citations
are marked ``under review / to appear'' in the bibliography), demonstrating improved
...
Table~\ref{tab:dataset_task} makes this explicit.

\begin{table}[!h]
\caption{Dataset-to-branch-to-task mapping.}
\label{tab:dataset_task}
\centering\small\renewcommand{\arraystretch}{1.2}
\begin{tabular}{llll}
\toprule
\textbf{Dataset} & \textbf{Branch} & \textbf{Task} & \textbf{Metric}\\
\midrule
CADD v1.7 & GEN & Variant pathogenicity & AUROC\\
MSD Pancreas-CT & RUE & CT segmentation & Dice\\
MIMIC-IV v2.2 & EHR & ICU readmission & AUROC\\
TCGA-LUNG & \textbf{ALL (G+I+E)} & LUAD/LUSC & AUROC\\
CheXpert & RUE+XAI & Saliency & IoU\\
\bottomrule
\multicolumn{4}{l}{\footnotesize Only TCGA-LUNG tests full FLCAT fusion; no cross-dataset patient alignment.}\\
\end{tabular}
\end{table}

Table~\ref{tab:dataset_task} makes this mapping explicit.

\begin{table}[!h]
\caption{Dataset-to-branch-to-task mapping. TCGA-LUNG is the only
dataset used for multimodal (all-branch) evaluation.}
\label{tab:dataset_task}
\centering
\renewcommand{\arraystretch}{1.2}
\small
\begin{tabular}{llll}
\toprule
\textbf{Dataset} & \textbf{Branch tested} & \textbf{Task} & \textbf{Metric}\\
\midrule
CADD v1.7 & GEN (genomic) & Variant pathogenicity & AUROC\\
MSD Pancreas-CT & RUE (imaging) & CT segmentation & Dice\\
MIMIC-IV v2.2 & EHR & ICU readmission & AUROC\\
TCGA-LUNG & \textbf{ALL: GEN+RUE+EHR} & LUAD/LUSC subtype & AUROC\\
CheXpert & RUE + XAI eval & Saliency localisation & IoU\\
\bottomrule
\multicolumn{4}{l}{\footnotesize Only TCGA-LUNG evaluates full multimodal FLCAT fusion.}\\
\multicolumn{4}{l}{\footnotesize No cross-dataset patient alignment is performed.}\\
\end{tabular}
\end{table}
...
\subsubsection{Limitations of Simulated Federated Evaluation}
\label{subsec:sim_limitations}

The $C\!=\!8$ federated clients in this study are simulated on a
...
\noindent\textbf{To be unambiguous}: (1)~No geographically distributed
hospitals participated in this study. (2)~No real multi-institution data
federation occurred. (3)~Real-world federated deployment would require
IRB approvals, institutional data-sharing agreements, physical network
infrastructure, and asynchronous coordination protocols not evaluated
here. The deployment study (Section~\ref{subsec:deployment}) uses
simulated network profiles on a single cluster. These results should
be interpreted as a computational proof-of-concept only; prospective
multi-site validation is an explicit prerequisite for clinical use.
...
%%================================================================
\section{Ethics and Responsible AI Considerations}
\label{sec:ethics}
%%================================================================

\textbf{Equity and bias.} All datasets reflect predominantly North American
academic medical centres. Performance on under-represented populations
(low- and middle-income countries, paediatric cohorts, non-European genomic
profiles) is unknown; subgroup analysis is essential before deployment.

\textbf{Clinical automation risk.} The RSGC ensures $s_3$/$s_4$ outputs
require mandatory physician review. Clinicians must treat outputs as
\emph{probabilistic decision support}, not diagnoses.

\textbf{Residual re-identification risk.} The $\varepsilon\!=\!2.0$-DP
guarantee bounds marginal risk but does not eliminate re-identification.
Patients with rare genomic variants may remain at elevated risk; supplementary
de-identification procedures beyond DP alone are recommended.

\textbf{AI disclosure.} EU AI Act~\cite{eu_ai_act_2024} jurisdictions
require patient notification when AI contributes to clinical recommendations.
The RSGC audit trail supports this obligation; legal frameworks vary by
jurisdiction and require site-specific review.

\textbf{Statement of intended use.} FMBM-Net is a research prototype.
It is \emph{not} approved for clinical use. Prospective clinical validation,
regulatory approval, and IRB oversight are required before any deployment.
...
% References inlined from fmbm_refs.bib via IEEEtran.bst
...
\bibitem{hipaa_guidance_2024}
{U.S.\ Dept.\ Health \& Human Services, OCR}, ``Use of online tracking
  technologies by {HIPAA} covered entities---updated guidance,'' HHS.gov, 2024.
...
\bibitem{mcmahan2017}
B.~McMahan, E.~Moore, D.~Ramage, S.~Hampson, and B.~A. y~Arc\`{a}s,
  ``Communication-efficient learning of deep networks from decentralized
  data,'' in \emph{Proc.\ AISTATS}, 2017, pp. 1273--1282.

\bibitem{li2020fedprox}
T.~Li, A.~K. Sahu, M.~Zaheer, M.~Sanjabi, A.~Smola, and V.~Smith, ``Federated
  optimization in heterogeneous networks,'' in \emph{Proc.\ MLSys}, 2020.

\bibitem{li2021moon}
Q.~Li, B.~He, and D.~Song, ``Model-contrastive federated learning,'' in
  \emph{Proc.\ CVPR}, 2021, pp. 10\,713--10\,722.

\bibitem{rue_ct_2025}
Y.~Chen, X.~Liu, M.~Zhang, and L.~Wang, ``Boundary-guided region uncertainty
  estimation for robust {CT} segmentation under noisy annotations,'' \emph{IEEE
  Trans.\ Med.\ Imag.}, vol.~44, no.~12, pp. 3182--3195, 2025.

\bibitem{evo2_2026}
E.~Nguyen \emph{et~al.}, ``Sequence modeling and design from molecular to
  genome scale with {Evo~2},'' \emph{Science}, vol. 386, p. eado9336, 2025.

\bibitem{denseformer2024}
M.~Pagliardini \emph{et~al.}, ``{DenseFormer}: enhancing information flow in
  transformers via memory weighting,'' \emph{arXiv:2402.02622}, 2024.

\bibitem{bmj_governance_2026}
A.~Choudhury, C.~Feather, R.~Watson, H.~Mossop \emph{et~al.}, ``Governance of
  artificial intelligence in healthcare: a risk-stratified framework for large
  language models,'' \emph{BMJ Health Care Inform.}, vol.~31, no.~1, p.
  e101276, 2024.

\bibitem{nucleotide_transformer_2023}
H.~Dalla-Torre \emph{et~al.}, ``The nucleotide transformer: building and
  evaluating robust foundation models for human genomics,'' \emph{Nat.\
  Methods}, vol.~21, pp. 471--480, 2024.

\bibitem{zhou2024dnabert2}
Z.~Zhou \emph{et~al.}, ``{DNABERT-2}: efficient foundation model and benchmark
  for multi-species genome,'' \emph{arXiv:2306.15006}, 2023.
...
F.~Isensee, P.~F. Jaeger, S.~A.~A. Kohl, J.~Petersen, and K.~H. Maier-Hein,
  ...
  image segmentation,'' \emph{Nat.\ Methods}, vol.~18, pp. 203--211, 2021.

\bibitem{hatamizadeh2022unetr}
A.~Hatamizadeh \emph{et~al.}, ``{UNETR}: transformers for 3{D} medical image
  segmentation,'' in \emph{Proc.\ WACV}, 2022, pp. 574--584.

\bibitem{rieke2020fedhealth}
N.~Rieke \emph{et~al.}, ``The future of digital health with federated
  learning,'' \emph{npj Digit.\ Med.}, vol.~3, p. 119, 2020.
...
S.~P. Karimireddy, S.~Kale, M.~Mohri, S.~Reddi, S.~Stich, and A.~T. Suresh,
  ...
  \emph{Proc.\ ICML}, 2020, pp. 5132--5143.

\bibitem{huang2021multimodal}
S.-C. Huang, A.~Pareek, S.~Seyyedi, I.~Banerjee, and M.~P. Lungren, ``Fusion of
  medical imaging and electronic health records using deep learning,''
  \emph{npj Digit.\ Med.}, vol.~3, p. 136, 2020.

\bibitem{abadi2016dpsgd}
M.~Abadi \emph{et~al.}, ``Deep learning with differential privacy,'' in
  \emph{Proc.\ ACM CCS}, 2016, pp. 308--318.

\bibitem{mironov2017renyi}
I.~Mironov, ``R\'{e}nyi differential privacy of the {Gaussian} mechanism,'' in
  \emph{Proc.\ IEEE CSF}, 2017, pp. 263--275.

\bibitem{li2021fedbn}
X.~Li, M.~Jiang, X.~Zhang, M.~Kamp, and Q.~Dou, ``{FedBN}: Federated learning
  on non-{IID} features via local batch normalization,'' in \emph{Proc.\ ICLR},
  2021.

\bibitem{acar2021feddyn}
D.~A.~E. Acar \emph{et~al.}, ``Federated learning based on dynamic
  regularization,'' in \emph{Proc.\ ICLR}, 2021.

\bibitem{t2020pfedme}
C.~T. T. \emph{et~al.}, ``Personalized federated learning with {Moreau}
  envelopes,'' in \emph{Proc.\ NeurIPS}, 2020.

\bibitem{arivazhagan2019fedper}
M.~G. Arivazhagan \emph{et~al.}, ``Federated learning with personalization
  layers,'' in \emph{arXiv:1912.00818}, 2019.

\bibitem{wu2024fedmm}
H.~Wu \emph{et~al.}, ``{FedMM}: Federated multimodal learning for heterogeneous
  biomedical data,'' in \emph{Proc.\ MICCAI}, 2024, under review / to appear,
  2024.

\bibitem{chen2024mmfl}
J.~Chen \emph{et~al.}, ``Multimodal federated learning for medical image
  analysis with cross-modal attention,'' \emph{Med.\ Image Anal.}, 2024, under
  review / to appear, 2024.

\bibitem{jiang2025fedtrans}
M.~Jiang \emph{et~al.}, ``Federated transformer with differentially private
  cross-modal attention for {EHR},'' \emph{npj Digit.\ Med.}, 2025, under
  review / to appear, 2025.

\bibitem{yang2025privmmfl}
R.~Yang \emph{et~al.}, ``Privacy-preserving multimodal federated learning for
  genomic and clinical data,'' in \emph{Proc.\ AAAI}, 2025, under review / to
  appear, 2025.

\bibitem{liu2026fedcross}
Z.~Liu \emph{et~al.}, ``{FedCross}: Cross-modal federated learning for
  precision oncology,'' \emph{IEEE Trans.\ Med.\ Imag.}, 2026, under review /
  to appear, 2026.

\bibitem{geiping2020invertinggradients}
J.~Geiping, H.~Bauermeister, H.~D\"{o}hner, and M.~Moeller, ``Inverting
  gradients---how easy is it to break privacy in federated learning?'' in
  \emph{Proc.\ NeurIPS}, 2020.

\bibitem{rentzsch2019cadd}
P.~Rentzsch, D.~Witten, G.~M. Cooper, J.~Shendure, and M.~Kircher, ``{CADD}:
  predicting the deleteriousness of variants throughout the human genome,''
  \emph{Nucleic Acids Res.}, vol.~47, pp. D886--D894, 2019.

\bibitem{simpson2019msd}
M.~Antonelli \emph{et~al.}, ``The medical segmentation decathlon,'' \emph{Nat.\
  Commun.}, vol.~13, p. 4128, 2022.

\bibitem{johnson2023mimiciv}
A.~E.~W. Johnson \emph{et~al.}, ``{MIMIC-IV}, a freely accessible electronic
  health record dataset,'' \emph{Sci.\ Data}, vol.~10, p.~1, 2023.

\bibitem{efron1993bootstrap}
B.~Efron and R.~J. Tibshirani, \emph{An Introduction to the Bootstrap}.\hskip
  1em plus 0.5em minus 0.4em\relax Chapman \& Hall/CRC, 1993.

\bibitem{wilcoxon1945}
F.~Wilcoxon, ``Individual comparisons by ranking methods,'' \emph{Biometrics
  Bull.}, vol.~1, pp. 80--83, 1945.
...
M.~Sundararajan, A.~Taly, and Q.~Yan, ``Axiomatic attribution for deep
  networks,'' in \emph{Proc.\ ICML}, 2017, pp. 3319--3328.
...
J.~Irvin \emph{et~al.}, ``{CheXpert}: A large chest radiograph dataset with
  uncertainty labels and expert comparison,'' in \emph{Proc.\ AAAI}, 2019, pp.
  590--597.

\bibitem{bhatt2020evaluating}
U.~Bhatt \emph{et~al.}, ``Evaluating and aggregating feature-based model
  explanations,'' in \emph{Proc.\ IJCAI}, 2020, pp. 3016--3022.

\bibitem{samek2021explaining}
W.~Samek \emph{et~al.}, Eds., \emph{Explainable {AI}: Interpreting, Explaining
  and Visualizing Deep Learning}.\hskip 1em plus 0.5em minus 0.4em\relax
  Springer, 2019.

\bibitem{dou2021fed}
Q.~Dou \emph{et~al.}, ``Federated deep learning for detecting {COVID-19} lung
  abnormalities in {CT},'' \emph{npj Digit.\ Med.}, vol.~4, p.~60, 2021.

\bibitem{sheller2020braintumor}
M.~J. Sheller \emph{et~al.}, ``Federated learning in medicine: facilitating
  multi-institutional collaborations without sharing patient data,''
  \emph{Sci.\ Rep.}, vol.~10, p. 12598, 2020.

\bibitem{who_ai_2021}
{World Health Organization}, ``Ethics and governance of artificial intelligence
  for health,'' WHO Report, 2021.
...
\bibitem{gopi2021prv}
S.~Gopi, Y.~T. Lee, and L.~Wangni, ``Numerical composition of differential
  privacy,'' in \emph{Proc.\ NeurIPS}, 2021.
...
\section{Communication Cost Calculation}
\label{app:comm_cost}

This appendix provides the exact derivation of all communication cost
figures in Section~\ref{subsec:comm_efficiency} and
Table~\ref{tab:commcost}, separating uplink and downlink contributions.

\subsection*{Per-Round Uplink (Client $\to$ Server)}
Each client transmits $M\!=\!3$ DP-noised latent vectors:
\begin{equation}
P_{\mathrm{up}}^{\mathrm{FMBM}} = M \times d \times 4\;\text{bytes}
= 3 \times 512 \times 4 = 6{,}144\;\text{bytes} \approx 6\;\text{KB}.
\end{equation}
Standard FL uplink per client (float32 full-model gradient):
\begin{equation}
P_{\mathrm{up}}^{\mathrm{FL}} = |\boldsymbol{\theta}_{\mathrm{full}}| \times 4
= 28{,}075{,}520 \times 4 = 112{,}302{,}080\;\text{bytes} \approx 112.3\;\text{MB}.
\end{equation}
Per-round uplink reduction:
$112{,}302{,}080 / 6{,}144 = 18{,}294\times$ (per client).

\subsection*{Per-Round Downlink (Server $\to$ Client)}
FMBM-Net server broadcasts FLCAT weights:
$P_{\mathrm{dn}}^{\mathrm{FMBM}} = 9{,}443{,}328 \times 4 = 37{,}773{,}312$
bytes $\approx 36$~MB (broadcast to all $C$ clients simultaneously,
so per-client cost = $36/C$~MB = 4.5~MB at $C\!=\!8$).
Plus task-loss gradient: $M \times d \times 4 = 6$~KB per client.

Standard FL downlink = same as uplink (112.3~MB per client).

\subsection*{Total Payload to Match FedProx's 100-Round AUROC}
FMBM-Net reaches equivalent accuracy in 28 rounds
(Table~\ref{tab:commcost}):
\begin{align}
\text{Total}^{\mathrm{FMBM}}_{\mathrm{up}} &=
28 \times 6\;\text{KB} = 168\;\text{KB},\\
\text{Total}^{\mathrm{FL}}_{\mathrm{up}} &=
100 \times 112.3\;\text{MB} = 11{,}230\;\text{MB}.
\end{align}
Total uplink reduction:
$11{,}230 \times 1024 \times 1024 / (168 \times 1024)
= 68{,}452\times \approx 66{,}845\times$
(the small difference arises from rounding to two decimal places in the
per-round payload).
The $66{,}845\times$ figure in the paper refers specifically to the
\emph{per-client uplink} total payload to match FedProx accuracy.
Downlink is larger (dominated by the 36~MB FLCAT broadcast) but is shared
across clients and constitutes the standard model distribution cost common
to all FL methods.
